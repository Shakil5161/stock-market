import React from 'react';

const Blog = () => {
    return (
        <>
            <div id="blog" className="py-16 sm:py-16 md:py-44 lg:py-12 px-4">
            <div className>
                <h1 className="xl:text-5xl pt-4 xl:pt-0 text-3xl text-gray-800 text-center font-extrabold mb-4">Our Blog Posts</h1>
                <p className="text-xl text-gray-600 text-center xl:w-3/5 mx-auto w-11/12">Don’t focus on having a great blog. Focus on producing a blog that’s great for your readers.” As a blogger, everything you do flows from understanding your audience and seeking to help them.</p>
            </div>
            <div className="lg:flex md:flex xl:justify-around sm:flex flex-wrap md:justify-around sm:justify-around lg:justify-around pt-8">
                <div className="xl:w-1/3 sm:w-5/12 sm:max-w-xs relative pb-16 lg:pb-10 xl:max-w-sm  lg:w-1/2 w-11/12 mx-auto sm:mx-0 animate__animated animate__fadeInLeftBig">
                    <div className="shadow h-64 rounded z-10">
                        <img src="https://cdn.tuk.dev/assets/photo-1573451444472-7b0b275ab824.jfif" alt className="h-full w-full object-cover overflow-hidden rounded" />
                    </div>
                    <div className="p-6 shadow-lg w-11/12 mx-auto -mt-20 bg-white text-center rounded z-20 relative">
                        <p className="uppercase text-sm text-indigo-700 pb-3">VEHICLE</p>
                        <p className="text-lg text-gray-800 text-center pb-3">Is It Worth Investing in Wireless Internet for Your Car?</p>
                        <a href="#">
                            <p className="hover:text-indigo-700 text-center  font-semibold animate__animated animate__pulse animate__infinite animate__slow">Read More</p>
                        </a>
                        <p className="text-sm text-gray-800 text-center">
                            May 13, 2019 by
                            <a href="javascript:void(0)">
                                <span className="text-indigo-700 cursor-pointer">Silene Cox</span>
                            </a>
                        </p>
                    </div>
                </div>
                <div className="xl:w-1/3 sm:w-5/12 sm:max-w-xs relative pb-16 lg:pb-10 xl:max-w-sm lg:w-1/2 w-11/12 mx-auto sm:mx-0 animate__animated animate__zoomIn">
                    <div className="shadow h-64 rounded">
                        <img src="https://cdn.tuk.dev/assets/photo-1544476915-ed1370594142.jfif" alt className="h-full w-full object-cover overflow-hidden rounded" />
                    </div>
                    <div className="p-6 shadow-lg w-11/12 mx-auto -mt-20 bg-white rounded z-20 relative">
                        <p className="uppercase text-sm text-indigo-700 text-center pb-3">Work</p>
                        <p className="text-lg text-gray-800 text-center pb-3">What Is Insolvent Trading?</p>
                        <a href="#">
                            <p className="hover:text-indigo-700 text-center  font-semibold animate__animated animate__pulse animate__infinite animate__slow">Read More</p>
                        </a>
                        <p className="text-sm text-gray-800 text-center">
                            May 13, 2019 by
                            <a href="javascript:void(0)">
                                <span className="text-indigo-700 cursor-pointer">Sansa Rollins</span>
                            </a>
                        </p>
                    </div>
                </div>
                <div className="xl:w-1/3 sm:w-5/12 sm:max-w-xs relative pb-16 lg:pb-10 xl:max-w-sm lg:w-1/2 w-11/12 mx-auto sm:mx-0 animate__animated animate__fadeInRightBig">
                    <div className="shadow h-64 rounded">
                        <img src="https://cdn.dnaindia.com/sites/default/files/styles/full/public/2019/08/21/860914-stockmarket-istock-111518.jpg" alt className="h-full w-full object-cover overflow-hidden rounded" />
                    </div>
                    <div className="p-6 shadow-lg w-11/12 mx-auto -mt-20 bg-white rounded z-20 relative">
                        <p className="uppercase text-sm text-indigo-700 text-center pb-3">Productivity</p>
                        <p className="text-lg text-gray-800 text-center pb-3">What Are the Risks of Getting GreenSky Loans?</p>
                        <a href="#">
                            <p className="hover:text-indigo-700 text-center  font-semibold animate__animated animate__pulse animate__infinite animate__slow">Read More</p>
                        </a>
                        <p className="text-sm text-gray-800 text-center">
                            May 13, 2019 by
                            <a href="javascript:void(0)">
                                <span className="text-indigo-700 cursor-pointer">Ashley Wilson</span>
                            </a>
                        </p>
                    </div>
                </div>
            </div>
        </div>
        </>
    );
};

export default Blog;